...
class TemporalAttentionLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=1024, dropout=0.1):
        ...
        # Sine-Cosine Position Encoding
        self.pos_encoding_curt = PositionEncodingSine(d_model)
        self.pos_encoding_prev = PositionEncodingSine(d_model)
    ...
    def forward(self, curt_feat, prev_feat, grd_feat_h, grd_feat_w):
        ...
        curt_feat = curt_feat + self.dropout1(curt_feat)
        curt_feat = self.norm1(curt_feat)
        curt_feat_ffn = self.FFN(curt_feat)
        curt_feat = curt_feat + self.dropout2(curt_feat_ffn)
        curt_feat = self.norm2(curt_feat)
